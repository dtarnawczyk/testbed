...
public class Rainfall {
    ...
    public static void main(String [] args) {
        ...
        System.out.println(mean("London", " "));
        System.out.println(variance("London", " x   "));
...
//        }
    ...
    public static double mean(String town, String strng) {
        List<Double> rainfallData = getRainfallData(town, strng);
        if(rainfallData.isEmpty()) return -1;
        double sum = 0;
        for(double dbl : rainfallData) {
            sum += dbl;
        }
    ...
    public static double variance(String town, String strng) {
        ...
        if(rainfallData.isEmpty()) return -1;
        double var = 0.0;
        for(double a : rainfallData) {
            var += (a - mean) * (a - mean);
        }
        return var/rainfallData.size();
    }

    private static List<Double> getRainfallData(String town, String strng){
        String [] data = strng.split("\n");
        String townData = "";
        Optional<String> optString = Arrays.stream(data).filter(s -> s.startsWith(town)).findFirst();
        if(optString.isPresent())
            townData = optString.get();
        else
            return Collections.emptyList();
        String [] rainfall = townData.substring(town.length()+1).split(",");
        return Arrays.stream(rainfall)
                .map(s -> Double.valueOf(s.substring(4)))
